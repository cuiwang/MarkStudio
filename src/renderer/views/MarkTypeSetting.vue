  ...
  <div id="content">
    <div class="marktype-header">
      <el-card shadow="never">
        <div class="flex_row">
          <div class="flex_1">
            <div style="font-size: 20px;color: #303133;text-align: left">标注类型</div>
            <div class="h10"></div>
            <div style="font-size: 14px;color: #909399;text-align: left">
              预制了常用的两种序列标注类型：BIO标注法和BIOES标注法。其他类型需要自定义.
            </div>
          </div>
          <div class="w10"></div>
          <div>
            <el-button type="primary" @click="needShowNewMarkTypeView = true">新建标注类型</el-button>
          </div>
        </div>
      </el-card>
    </div>
        ...
        <el-table-column width="200">
          ...
          <template slot-scope="scope">
            <div class="flex_row">
              ...
              <el-button
                :disabled="scope.row.editable === false"
                size="mini"
                @click="editMarkType(scope.$index, scope.row)"
                >编辑
              </el-button>
              <el-button
                :disabled="scope.row.editable === false"
                size="mini"
                type="danger"
                @click="deleteMarkType(scope.$index, scope.row)"
                >删除
              </el-button>
            </div>
          </template>
    ...
    <el-dialog
      title="新建标注类型"
      :visible.sync="needShowNewMarkTypeView"
      :show-close="false"
      ...
      <el-form :model="newMarkTypeForm" label-position="left" label-width="80px">
            ...
            <div v-show="newMarkTypeForm.datas.length > 0">
              <div class="tag-warp">
                <div v-for="(data, index) in newMarkTypeForm.datas" :key="index" class="tag-warp-item">
                  <el-tag closable @close="handleNewMarkTypeTagClose(data, index)"
                    >{{ `${data.tag}-${data.name}` }}
                  </el-tag>
                </div>
              </div>
        ...
        <el-form-item label="描述">
          <el-input
            type="textarea"
            clearable
            v-model="newMarkTypeForm.description"
    ...
    <el-dialog
      title="修改标注类型"
      :visible.sync="needShowEditMarkTypeView"
      :show-close="false"
      :close-on-click-modal="false"
      :close-on-press-escape="false"
    >
      <el-form :model="editMarkTypeForm" label-position="left" label-width="80px">
            ...
            <div v-show="editMarkTypeForm.datas.length > 0">
              <div class="tag-warp">
                <div v-for="(data, index) in editMarkTypeForm.datas" :key="index" class="tag-warp-item">
                  <el-tag closable @close="handleEditMarkTypeTagClose(data, index)"
                    >{{ `${data.tag}-${data.name}` }}
                  </el-tag>
                </div>
              </div>
        ...
        <el-form-item label="描述">
          <el-input
            type="textarea"
            clearable
            v-model="editMarkTypeForm.description"
            placeholder="请填写描述,方便理解和解释"
            maxlength="200"
            show-word-limit
          ></el-input>
...
export default {
  ...
  mounted() {
    console.log('MarkTypeSetting nounted');
