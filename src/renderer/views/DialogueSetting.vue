<template>
  <div id="content">
    ...
    <!--新建实体标注标签组-->
    <el-dialog width="45%" title="新建-对话标注" :visible.sync="needShowNewMarkTypeView" :show-close="false" :close-on-click-modal="false" :close-on-press-escape="false">
    ...
    <!--修改实体标注标签组-->
    <el-dialog width="45%" title="修改-对话标注" :visible.sync="needShowEditMarkTypeView" :show-close="false" :close-on-click-modal="false" :close-on-press-escape="false">
...
export default {
  ...
  mounted() {
    console.log('DialogueSetting mounted');
  ...
  methods: {
    initData() {
      db_utils
        .generate_find(db_utils.DIALOGUE_TYPES_DB, {})
        .sort({ editable: 1 })
        .exec((err, documents) => {
    ...
    saveMarkType() {
      if (this.needShowNewMarkTypeView && this.newMarkTypeForm.datas.length > 0) {
        ...
        // 保存到数据库
        db_utils.insert(db_utils.DIALOGUE_TYPES_DB, this.newMarkTypeForm, (err, newDoc) => {
          //console.log('marktype new:' + newDoc);
        });
        this.clearNewMarkTypeForm();
        this.$events.emit('DIALOGUESETTING_CHANGED', 'MARKTYPE');
      } else {
        this.$message({
          message: '请至少填写一个标签',
          type: 'error',
    ...
    saveEditMarkType() {
      if (this.needShowEditMarkTypeView && this.editMarkTypeForm.datas.length > 0) {
        ...
        // 保存到数据库
        db_utils.update(db_utils.DIALOGUE_TYPES_DB, { _id: this.editMarkTypeForm._id }, this.editMarkTypeForm, {
          upsert: false,
        });
    ...
    deleteMarkType(index, row) {
      this.$confirm('此操作将永久删除该配置, 是否继续?', '提示', {
        ...
        .then(() => {
          ...
          // 从数据库删除
          db_utils.remove(db_utils.DIALOGUE_TYPES_DB, { _id: row._id }, {}, (err, n) => {
